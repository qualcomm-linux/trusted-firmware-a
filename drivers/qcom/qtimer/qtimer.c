...
void qcom_qtimer_init(void)
{
	...
	mmio_write_32(QCOM_QTIMER_BASE + SAR_FG0_OFFSET, 0x7F);

	for (i = 0; i < QTIMER_NBR_FRAMES; i++) {
		mmio_write_32(QCOM_QTIMER_BASE + ACR_FG0_OFFSET(i), 0x3F);
		mmio_write_32(QCOM_QTIMER_BASE + VOFF_FG0_LO_OFFSET(i), 0x0);
		mmio_write_32(QCOM_QTIMER_BASE + VOFF_FG0_HI_OFFSET(i), 0x0);
	}
...
{
	uint64_t ticks = qcom_qtimer_get_raw();
