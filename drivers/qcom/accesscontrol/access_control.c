...
process_sources(const uint32_t *src_vm_list, uint32_t src_vm_count,
		const qcom_accesscontrol_perm_t *dst_vm_list,
		uint32_t dst_vm_count, enum device_type *device,
		enum domain_type *domain)
{
	const uint32_t *src_vm;
	const qcom_accesscontrol_perm_t *dst_vm;
	const qcom_accesscontrol_perm_t *dst_vm_limit;
	uint32_t src_index;
	uint32_t dst_index;
...
static enum ac_error assign_regions(const qcom_accesscontrol_mem_t *mem_regions,
				    ...
				    uint32_t write_perm_domain)
{
	const qcom_accesscontrol_mem_t *mem_region = mem_regions;
	uint32_t mem_index;
...
static uint64_t mem_assign(const qcom_accesscontrol_mem_t *mem_regions,
			   uint32_t mem_region_count,
...
static int xpu_register_interrupts(void)
{
	...
	err = qcom_interrupt_svc_register(QTISECLIB_INT_ID_XPU_SEC, xpu_isr,
					 &xpu_err_sec_ctx);
	if (err)
		return err;

	err = qcom_interrupt_svc_register(QTISECLIB_INT_ID_XPU_NON_SEC, xpu_isr,
					 &xpu_err_non_sec_ctx);
	if (err)
		qcom_interrupt_svc_unregister(QTISECLIB_INT_ID_XPU_SEC);
...
uint64_t qcom_accesscontrol_mem_assign(const qcom_accesscontrol_mem_t *mem,
				      uint32_t mem_len, const uint32_t *src,
				      uint32_t src_len,
				      const qcom_accesscontrol_perm_t *perm,
				      uint32_t perm_len)
{
	return mem_assign(mem, mem_len, src, src_len, perm, perm_len);
}

void qcom_accesscontrol_init(void)
{
	int rc;
