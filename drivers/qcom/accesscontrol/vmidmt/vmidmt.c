...
static int32_t register_interrupts(void)
{
	...
	ret = qcom_interrupt_svc_register(QTISECLIB_INT_ID_VMIDMT_ERR_CLT_SEC,
					 error_handler, &vmidmt_err[CLT_SEC]);
	if (ret)
		return ret;

	ret = qcom_interrupt_svc_register(QTISECLIB_INT_ID_VMIDMT_ERR_CLT_NONSEC,
					 error_handler,
					 &vmidmt_err[CLT_NONSEC]);
	if (ret)
		goto error3;

	ret = qcom_interrupt_svc_register(QTISECLIB_INT_ID_VMIDMT_ERR_CFG_SEC,
					 error_handler, &vmidmt_err[CFG_SEC]);
	if (ret)
		goto error2;

	ret = qcom_interrupt_svc_register(QTISECLIB_INT_ID_VMIDMT_ERR_CFG_NONSEC,
					 error_handler,
...
error1:
	qcom_interrupt_svc_unregister(QTISECLIB_INT_ID_VMIDMT_ERR_CFG_SEC);
error2:
	qcom_interrupt_svc_unregister(QTISECLIB_INT_ID_VMIDMT_ERR_CLT_NONSEC);
error3:
	qcom_interrupt_svc_unregister(QTISECLIB_INT_ID_VMIDMT_ERR_CLT_SEC);
