...
uint64_t qcom_watchdog_pet_ticks __section("tzfw_coherent_mem");
...
void qcom_watchdog_start(uint32_t bark_ms, uint32_t bite_ms)
{
	uint32_t bark, bite;
...
{
	qcom_watchdog_pet_ticks = qcom_qtimer_get_raw();
	mmio_write_32(WDOG_RESET_ADDR, RESET);
	dsb();
}
...
void qcom_watchdog_set_target(qcom_watchdog_cpu_state_t state)
{
	...
	case QCOM_WATCHDOG_CPU_WAKEUP:
		...
		break;
	case QCOM_WATCHDOG_CPU_HOTPLUG:
		cpu.state.interruptible &= ~(1 << core);
		...
		break;
	case QCOM_WATCHDOG_CPU_SUSPEND:
		if (cpu.watchdog != core) {
			cpu.state.on &= ~(1 << core);
			break;
		}
...
int qcom_watchdog_init(void)
{
	...
	ret = qcom_interrupt_svc_register(WDOG_BARK_INT_ID, bark_handler, NULL);
	if (ret) {
		ERROR("Failure registering watchdog\n");
		return ret;
	}

	qcom_watchdog_set_target(QCOM_WATCHDOG_CPU_WAKEUP);
	qcom_watchdog_start(WDOG_BARK_TIME_MS, WDOG_BITE_TIME_MS);
