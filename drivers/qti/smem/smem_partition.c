...
static struct smem_partition_info *smem_part_info_alloc(struct smem_toc *toc,
						      uint32_t *part_info_cnt)
{
...
static struct smem_partition_info *smem_part_info_get(smem_host_type remote_host)
{
	uint32_t indx;
...
void smem_part_init(void)
{
	...
	smem_info_prt = smem_part_info_alloc(toc, &smem_info_num_prt);
	if (!smem_info_prt) {
		ERROR("SMEM: assert failed in %s() at line %d\n",
		      __func__, __LINE__);
		plat_error_handler(EFAULT);
	...
	for (i = 0, part_indx = 0;
		...
		if (!smem_part_is_host_present(entry, this_host)) {
			continue;
		}
		...
		if (part_size != size) {
			ERROR("SMEM: invalid partition size(%d). Actual partition size:%d, part indx: %d\n",
			      part_size, size, i);
			continue;
		}
