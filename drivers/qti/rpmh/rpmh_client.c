 ...
 *
 * RPMh command service for the lemans (qcs9075) APSS RSC, TZ DRV.
 *
...
#include "lemans/rsc_regs.h"

/*
 * TZ DRV TCS configuration (matches the qcs9075 RPMh DRV config):
...
static struct rpmh_client_t rpmh_clients[RSC_DRV_MAX];
...
{
	struct rpmh_client_t *client;

	assert(rpmh_initialized);
	assert(drv_id < RSC_DRV_MAX);
 ...
 */
static uint32_t rpmh_send_amc(struct rpmh_client_t *client,
			      const rpmh_command_set_t *cmd_set)
{
