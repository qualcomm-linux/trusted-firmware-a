...
{
	if (!valid_resource_idx(resource_idx)) {
		return -1;
	}

	return (int)resource_list[resource_idx].count;
}
...
int pwr_utils_named_resource_lvls_count(const char *resource)
{
	...
	return pwr_utils_resource_lvls_count(res_idx);
}
...
int pwr_utils_vlvl(int resource_idx, int hlvl)
{
	...
	if (!valid_resource_idx(resource_idx) || (hlvl < 0)) {
...
{
	return pwr_utils_vlvl(pwr_utils_lvl_resource_idx(resource), hlvl);
}
