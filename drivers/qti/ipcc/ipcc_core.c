		     ...
		     uint16_t signal_low, uint16_t signal_high)
{
	...
	/*
	 * Reject any client this chipset does not wire on this protocol. The same
	 * lookup yields the entry, so the legacy decision below needs no second
	 * pass over the table.
	 *
...
#if QTI_IPCC_LEGACY
	/*
	 * Dual-mode build: pick the mechanism this client is wired for. Without
	 * QTI_IPCC_LEGACY there is nothing to select and the router is the only
	 * path, which is why both flags are compiled out there.
	 */
	if (proto->legacy_supported && target->use_legacy_by_default) {
		return ipcc_legacy_trigger(target_id, signal_low, signal_high);
	}

	if (!proto->ipcc_mode_supported) {
		VERBOSE("IPCC: trigger unsupported for target %u\n",
			(uint32_t)target_id);
		return -ENOTSUP;
	}
#endif
