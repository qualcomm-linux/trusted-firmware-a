...
/*
 * Nord packs its client pages: the architectural client space runs to 72, but the
 * controller removes the pages of every client this chipset does not wire and
 * packs the 25 that remain contiguously from page 0. A client's physical index is
 * therefore its position in this array, not its client ID, and both the register
 * address and the SEND register's CLIENT_ID field take that index.
 *
...
#if QTI_IPCC_LEGACY
/* Nord does not use the legacy path for any client. */
