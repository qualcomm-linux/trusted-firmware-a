...
#include <drivers/qti/qtimer/qtimer_ncc.h>
...
static bool qtimer_ncc_init_done;
...
int qti_qtimer_ncc_init(void)
{
	...
	ret = qti_gic_register_isr(TIMER_SEC_QTMR_FR3_INT_ID,
				   qti_qtimer_ncc_sleep_isr, NULL);
	if (ret != 0) {
		return ret;
	}

	ret = qti_gic_register_isr(TIMER_SEC_QTMR_FR4_INT_ID,
				   qti_qtimer_ncc_sleep_isr, NULL);
	if (ret != 0) {
		qti_gic_unregister_isr(TIMER_SEC_QTMR_FR3_INT_ID);
		return ret;
	}

	ret = qti_gic_register_isr(TIMER_SEC_QTMR_FR5_INT_ID,
				   qti_qtimer_ncc_sleep_isr, NULL);
	if (ret != 0) {
		qti_gic_unregister_isr(TIMER_SEC_QTMR_FR4_INT_ID);
		qti_gic_unregister_isr(TIMER_SEC_QTMR_FR3_INT_ID);
		return ret;
	}

	qtimer_ncc_init_done = true;
...
/*
 * Marks the cluster sleep timer service ready after coldboot QTimer init.
 */
...
/*
 * Marks this core's CP15 sleep timer service ready after coldboot QTimer init.
 */
