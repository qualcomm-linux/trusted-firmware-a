...
#ifndef QTI_MBOX_QMP_H
#define QTI_MBOX_QMP_H
 ...
 * --------------------------------
 *   MCORE_LINK_STATE_ACK and MCORE_CH_STATE_ACK are written by SCORE (local).
 *   SCORE_LINK_STATE_ACK and SCORE_CH_STATE_ACK are written by MCORE (remote).
 *
 * MTU
 * ---
 * The effective TX MTU equals local_payload_size, which is valid only after
 * layout discovery (state >= LINK_NEGOTIATION).  qti_mbox_get_mtu() returns
 * -EAGAIN before the MTU is valid.
 *
 * Trust model
 * -----------
 ...
 * assumed to be correct; no runtime range validation is performed on them.
 */
...
/*
 * Signal configuration for notifying the remote endpoint.
 *
 * reg:   physical address of the interrupt-set register.
 *        Set to 0 to disable signaling (polling-only mode).
 * value: value to write to reg when signaling the remote.
 */
struct qti_mbox_signal_config {
	uintptr_t reg;
	uint32_t value;
};

/*
 * QMP transport configuration (immutable, supplied by platform code).
 *
 * desc_base:     physical base address of the shared QMP descriptor and
 ...
 *
 * These states are part of the QMP priv structure and are exposed here so
 * that platform code can allocate struct qti_mbox_qmp_priv statically.
 * Platform code must not read or write the state field directly.
 */
enum qti_mbox_qmp_state {
...
/*
 * QMP transport runtime state (mutable, one instance per channel).
 *
 * Platform code must allocate one instance of this structure per QMP channel
 * and pass a pointer to it via qti_mbox_chan_config.transport_priv.  The
 * framework and transport backend manage all fields; platform code must not
 ...
 * tx_pending:           true after qti_mbox_send() until the remote clears
 *                       msg_len (consuming the message); cleared when
 *                       QTI_MBOX_EVT_TX_DONE is generated or on disconnect.
 ...
 * local_payload_size:   usable TX capacity
 *                       (local_mbox_size - sizeof(msg_len)).
 * remote_payload_size:  usable RX capacity
 *                       (remote_mbox_size - sizeof(msg_len)).
 */
struct qti_mbox_qmp_priv {
	const struct qti_mbox_qmp_config *cfg;
	enum qti_mbox_qmp_state state;
	bool layout_valid;
	bool tx_pending;
 ...
 *
 * Pass &qti_mbox_qmp_ops as the ops field in qti_mbox_chan_config to use
 * the QMP transport backend.
 */
struct qti_mbox_ops;
extern const struct qti_mbox_ops qti_mbox_qmp_ops;

#endif /* QTI_MBOX_QMP_H */
