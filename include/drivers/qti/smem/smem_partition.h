...
/*
 * Debug info for the SMEM partition code
 */
struct smem_partition_debug_info {
	/*
	 ...
	 */
	uint32_t num_scan_uncached;
	/*
	 * Number of times this endpoint has detected one or more new SMEM items
	 * allocated by the remote endpoint and performed a scan
	 */
	uint32_t num_scan_cached;
};
...
struct smem_partition_info {
	 ...
	 */
	uint32_t offset_free_cached;
#if defined(SMEM_FEATURE_DEBUG)
	/* Debug info */
	struct smem_partition_debug_info debug_info;
#endif /* SMEM_FEATURE_DEBUG */
};
...
/*
 * Request a pointer to a buffer in shared memory.
 *
 * Upon success, params.buffer points to the allocation in shared memory.
 */
int32_t smem_part_alloc_ex(struct smem_alloc_params *params);
...
/*
 * Request a pointer to an item in shared/external memory.
 *
 * If the space for the item has been allocated in either shared/external
 * memory, return the pointer to that location, otherwise allocate buffer
 * for the specified item from the shared memory. For pre-allocated buffers,
 * an ERR_FATAL will occur if the buffer size requested does not match the
 * size of the existing buffer. If buf_size is not 64-bit aligned, this
 * function will round the size up to an aligned one.
 */
void *smem_part_alloc(enum smem_mem_type smem_type, uint32_t buf_size);
