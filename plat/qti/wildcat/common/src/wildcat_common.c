...
bool qti_is_overlap_bl31_region(unsigned long long addr, size_t size)
{
	if (addr > addr + size ||
				...
				unsigned int attr)
{
	...
	if (qti_is_overlap_bl31_region(aligned_pa, aligned_size)) {
...
int32_t plat_get_soc_version(void)
{
	 ...
	 */
	uint32_t soc_version = (chipinfo_get_chip_id() & SOC_ID_IMPL_DEF_MASK);
...
{
	uint32_t revision_no = chipinfo_get_chip_version();
	uint32_t major_rev = (revision_no >> 16) & U(0xFF);
	uint32_t minor_rev = revision_no & U(0xFF);
