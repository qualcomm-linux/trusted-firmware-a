 ...
 * are configured by qtimer_ncc.c on Hoya NCC targets.
 */
...
int qti_qtimer_platform_init(void)
{
	int ret;
	...
	ret = qti_gic_register_isr(TIMER_SEC_CP15_INT_ID, cp15_timer_isr,
				   NULL);
	if (ret != 0) {
		return ret;
	}

	ret = qti_gic_register_isr(TIMER_SEC_QTMR_INT_ID, qtmr_frame2_isr,
				   NULL);
	if (ret != 0) {
		qti_gic_unregister_isr(TIMER_SEC_CP15_INT_ID);
		return ret;
	}
