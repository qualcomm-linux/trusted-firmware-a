 ...
 * qcom_configure_mmu_elx() will give the available subset of that,
 */

const mmap_region_t plat_qcom_mmap[] = {
	MAP_REGION_FLAT(QCOM_DEVICE_BASE, QCOM_DEVICE_SIZE,
			MT_DEVICE | MT_RW | MT_SECURE),
	MAP_REGION_FLAT(QCOM_AOP_CMD_DB_BASE, QCOM_AOP_CMD_DB_SIZE,
			MT_NS | MT_RO | MT_EXECUTE_NEVER),
#ifdef QCOM_SMEM_BASE
	MAP_REGION_FLAT(QCOM_SMEM_BASE, QCOM_SMEM_SIZE,
			MT_NON_CACHEABLE | MT_RW | MT_EXECUTE_NEVER),
#endif
	{0}
};

CASSERT(ARRAY_SIZE(plat_qcom_mmap) <= MAX_MMAP_REGIONS, assert_max_mmap_regions);
...
/* TA region */
#if defined(QCOM_PIMEM_BASE) && defined(QCOM_PIMEM_LIMIT)
	{ QCOM_PIMEM_BASE, QCOM_PIMEM_LIMIT },
#endif
#endif
...
static bool qcom_is_overlap_secure_rg(uintptr_t addr, size_t size)
{
...
/*
 *  unsigned int plat_qcom_my_cluster_pos(void)
			  ...
			  )
{
	...
	mmap_add(plat_qcom_mmap);

	/* Create the page tables to reflect the above mappings */
	init_xlat_tables();
}

static inline void qcom_align_mem_region(uintptr_t addr, size_t size,
...
int qcom_mmap_add_dynamic_region(uintptr_t base_pa, size_t size,
				unsigned int attr)
{
	uintptr_t aligned_pa;
	size_t aligned_size;

	qcom_align_mem_region(base_pa, size, &aligned_pa, &aligned_size);

	if (qcom_is_overlap_secure_rg(base_pa, size)) {
...
int qcom_mmap_remove_dynamic_region(uintptr_t base_va, size_t size)
{
	qcom_align_mem_region(base_va, size, &base_va, &size);
...
{
	uint32_t soc_version = (chipinfo_get_chip_id() & QCOM_SOC_VERSION_MASK);
	uint32_t jep106az_code = (JEDEC_QCOM_BKID << QCOM_SOC_CONTINUATION_SHIFT)
			 | (JEDEC_QCOM_MFID << QCOM_SOC_IDENTIFICATION_SHIFT);
...
{
	return mmio_read_32(QCOM_SOC_REVISION_REG) & QCOM_SOC_REVISION_MASK;
}
