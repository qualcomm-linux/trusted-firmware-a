...
#define QCOM_INTR_INVALID_INT_NUM		0xFFFFFFFFU
#define ISR_TABLE_LEN				20

static struct qcom_isr_table {
	struct qcom_isr {
		qcom_int_svc_isr_t func;
		uint32_t id;
		void *ctx;
	} entry[ISR_TABLE_LEN];
	...
	.entry[0 ... ISR_TABLE_LEN - 1] = { .id = QCOM_INTR_INVALID_INT_NUM },
};

int qcom_interrupt_svc_register(uint32_t id, qcom_int_svc_isr_t func, void *ctx)
{
	struct qcom_isr *p = isr_table.entry;
	...
	for (size_t i = 0; i < ISR_TABLE_LEN; i++, p++) {
		if (p->id != QCOM_INTR_INVALID_INT_NUM) {
			continue;
		}
...
int qcom_interrupt_svc_unregister(uint32_t id)
{
	...
	for (size_t i = 0; i < ISR_TABLE_LEN; i++, p++) {
		...
		memset(p, 0, sizeof(*p));
		p->id = QCOM_INTR_INVALID_INT_NUM;
		isr_table.cnt--;
		spin_unlock(&isr_table.lock);
...
{
	struct qcom_isr *p = isr_table.entry;
	qcom_int_svc_isr_t invoke_isr = NULL;
...
plat_dispatch:
	spin_unlock(&isr_table.lock);
	plat_qcom_invoke_unhandled_isr(id, handle);
}

/*
 * Top-level EL3 interrupt handler.
 */
static uint64_t qcom_el3_interrupt_handler(uint32_t id, uint32_t flags,
					  void *handle, void *cookie)
{
	uint32_t irq = QCOM_INTR_INVALID_INT_NUM;
...
int qcom_interrupt_svc_init(bool have_sel1)
{
	...
	ret = register_interrupt_type_handler(INTR_TYPE_EL3,
					      qcom_el3_interrupt_handler, flags);
	assert(ret == 0);
