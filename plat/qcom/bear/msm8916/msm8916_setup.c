...
{
	const struct uartdm_gpios *gpios = &uartdm_gpio_map[QCOM_UART_NUM - 1];

	CASSERT(QCOM_UART_NUM > 0 && QCOM_UART_NUM <= ARRAY_SIZE(uartdm_gpio_map),
		assert_qcom_blsp_uart_valid);
	...
	mmio_setbits_32(GCC_BLSP1_UART_APPS_CBCR(QCOM_UART_NUM), CLK_ENABLE);
	while (mmio_read_32(GCC_BLSP1_UART_APPS_CBCR(QCOM_UART_NUM)) & CLK_OFF) {
	}
}
