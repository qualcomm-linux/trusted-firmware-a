					 ...
					 qtiseclib_mmap_attr_t attr)
{
	...
	return qcom_mmap_add_dynamic_region(base_pa, size, l_attr);
}

int qtiseclib_cb_mmap_remove_dynamic_region(uintptr_t base_va, size_t size)
{
	return qcom_mmap_remove_dynamic_region(base_va, size);
}
#endif
