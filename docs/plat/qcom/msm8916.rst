...
.. note::
	Unlike the :doc:`Qualcomm SC7180/SC7280 <chrome>` ports, this port does
 ...
 * ``QCOM_UART_NUM``: Number of UART controller to use for debug output and crash
 ...
 * ``QCOM_RUNTIME_UART``: By default (``0``) the UART is only used for the boot
